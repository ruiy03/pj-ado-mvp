    ...
    <div className="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 gap-3">
        <div className="flex-1">
          <h3 className="text-lg font-semibold text-gray-900 mb-2 break-words">{template.name}</h3>
      ...
      <div className="space-y-3">
        <div>
          ...
          <div className="bg-gray-50 p-2 rounded border overflow-hidden">
            <code className="text-sm text-gray-800 break-words whitespace-pre-wrap">{template.url_template}</code>
          </div>
        </div>
        ...
        {(() => {
          ...
          return Object.keys(urlParams).length > 0 && (
            <div>
              ...
              <div className="bg-gray-50 p-2 rounded border space-y-1 overflow-hidden">
                {Object.entries(urlParams).map(([key, value]) => (
                  <div key={key} className="text-sm flex flex-col sm:flex-row sm:items-center gap-1">
                    <span className="text-blue-600 font-medium break-words">{key}</span>
